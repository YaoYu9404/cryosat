/*  cryosat20hz_sard.c
 ...
 Modified by Yao Yu on July 14 2021.
 */
...
int main (int argc, char **argv) {
    ...
    if (n_out == 0) {
        fprintf (stderr, "usage: cryosat20hz_sard file1.nc file2.nc ... > output_binary_cryosat20hz_structures\n");
        exit (EXIT_FAILURE);
    }
    fprintf (stderr, "cryosat20hz_sard wrote %zu records to stdout.\n", n_out);
    exit (EXIT_SUCCESS);
}
...
size_t    handle_one_file (char *fname) {
    ...
    work = malloc (4 * 256 * n20hz_ku); /* Enough space to hold waveforms also */
    ...
    for (k = 0; k < n20hz_ku; k++) data[k].n_echo = us[k];
    ...
    for (k = 0; k < n20hz_ku; k++) data[k].hdopp = (short)i[k];
